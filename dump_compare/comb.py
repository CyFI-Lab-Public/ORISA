...
dump_directory = "/home/cyfi/aisha/cleaniris"
...
def analyze_with_r2(file_path, num):
    ...
    try:
        ...
        r2.quit()
        with open(os.path.join(dump_directory, f"read{num}.txt"), "w") as report:
...
def merge_files(file_list, output_prefix):
    part_num = 1
    output_path = os.path.join(dump_directory, f"{output_prefix}_part{part_num}.data")
...
# Output merged files
merged_files = glob.glob(os.path.join(dump_directory, "combined_*.data"))
merged_files

count = 0
for merged_file in merged_files:
    print(f"Analyzing {merged_file}...")
    analyze_with_r2(merged_file, count)
    count = count + 1

report_files = glob.glob(os.path.join(dump_directory, "read*.txt"))
report_files.sort()
with open(os.path.join(dump_directory, "readable.txt"), "w") as combined:
